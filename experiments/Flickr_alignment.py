...
class Flickr30kMultiModalTest():
    def __init__(self, encoder_type="clip"):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print("Device:", self.device)

        if encoder_type:
            self.image_embed_path = f"data/embeddings/flickr30k/{encoder_type}/image/train"
            self.image_test_embed_path = f"data/embeddings/flickr30k/{encoder_type}/image/test"
            self.text_embed_path = f"data/embeddings/flickr30k/{encoder_type}/text/train"
            self.text_test_embed_path = f"data/embeddings/flickr30k/{encoder_type}/text/test"
        else:
    ...
    def foundation_size_estimate(self, sample_num, encoder_type, label_type, ratios, train_set, test_set, num_class, write_hs=False):
        ...
        if write_hs:
            with open(f"src/scar/hs_list_flickr30k_{encoder_type}_{write_hs}.json", "w") as f:
                json.dump(ratio, f)
                f.write("\n")
                json.dump(step_hs, f)
...
def single_modal_main(modal_type, encoder_type, label_type):
    print(f"\nProcessing Flickr30k dataset with encoder {encoder_type} in {modal_type} modal.")
    classifier = Flickr30kMultiModalTest(encoder_type)
    num_class = 10
...
def main(encoder_type, label_type):
    ...
    result_save_path = f"experiments/results/flickr_experiment_results.txt"
    with open(result_save_path, 'a') as f:
        f.write("=" * 70 + "\n")
        f.write(f"  Flickr30k - Encoder: {encoder_type}; Label Type: {label_type}\n")
        f.write("=" * 70 + "\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Flickr30k Alignment experiments.")
    ...
    # python3 experiments/Flickr_alignment.py --encoder clip --label origin