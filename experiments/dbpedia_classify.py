...
# DBpedia Ontology: 630,000 samples


class DBpediaClassifyTest():
    def __init__(self, encoder_type="bert"):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print("Device:", self.device)

        if encoder_type == "bert":
            self.embed_path = "data/embeddings/dbpedia/bert/train"
            self.test_embed_path = "data/embeddings/dbpedia/bert/test"
        elif encoder_type == "roberta":
            self.embed_path = "data/embeddings/dbpedia/roberta/train"
            self.test_embed_path = "data/embeddings/dbpedia/roberta/test"
        elif encoder_type == "gpt2":
            self.embed_path = "data/embeddings/dbpedia/gpt2/train"
            self.test_embed_path = "data/embeddings/dbpedia/gpt2/test"
        else:
    ...
    def load_embeddings(self, path):
        # load_batches = [os.path.join(path, fname) for fname in os.listdir(path) if fname != "label_distribution.json"][:20] # Limit to 20 files for testing
    ...
    def foundation_size_estimate(self, sample_num, encoder_type, label_type, ratios, train_set, test_set, num_class, write_hs=False):
        ...
        if write_hs:
            with open(f"src/scar/hs_list_dbpedia_{encoder_type}_{write_hs}.json", "w") as f:
                json.dump(ratio, f)
                f.write("\n")
                json.dump(step_hs, f)
...
def main(encoder_type, label_type):
    classifier = DBpediaClassifyTest(encoder_type)
    num_class = 14
    ...
    result_save_path = "experiments/results/dbpedia_experiment_results.txt"
    with open(result_save_path, 'a') as f:
        f.write("=" * 70 + "\n")
        f.write(f"  DBpedia Ontology - Encoder: {encoder_type}; Label Type: {label_type}\n")
        f.write("=" * 70 + "\n")
...
#     encoder_type = "bert"  # Define the encoder types you want to test
#     main(encoder_type, label_type)
    ...
    # python3 experiments/dbpedia_classify.py