...
class CIFAR100ClassifyTest():
    def __init__(self, encoder_type="resnet"):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print("Device:", self.device)

        if encoder_type == "resnet":
            self.embed_path = "data/embeddings/cifar100/resnet/cifar100_embeddings.json"
            self.test_embed_path = "data/embeddings/cifar100/resnet/cifar100_test_embeddings.json"
        elif encoder_type == "vit":
            self.embed_path = "data/embeddings/cifar100/vit/cifar100_embeddings.json"
            self.test_embed_path = "data/embeddings/cifar100/vit/cifar100_test_embeddings.json"
        elif encoder_type == "dino":
            self.embed_path = "data/embeddings/cifar100/dino/cifar100_embeddings.json"
            self.test_embed_path = "data/embeddings/cifar100/dino/cifar100_test_embeddings.json"
        else:
    ...
    def foundation_size_estimate(self, sample_num, encoder_type, label_type, ratios, train_set, test_set, write_hs=False):
        ...
        ratio = (1 - primary_train_acc) / (1 - step_avg_acc) if step_avg_acc < 1 else np.e
        ...
        if write_hs:
            with open(f"src/scar/hs_list_cifar100_{encoder_type}_{write_hs}.json", "w") as f:
                json.dump(ratio, f)
                f.write("\n")
                json.dump(step_hs, f)
...
def main(encoder_type, label_type, split_ratio=0.6):
    classifier = CIFAR100ClassifyTest(encoder_type)
    ...
    result_save_path = "experiments/results/cifar100_experiment_results.txt"
    with open(result_save_path, 'a') as f:
        f.write("=" * 70 + "\n")
        f.write(f"  CIFAR100 - Encoder: {encoder_type}; Label Type: {label_type}\n")
        f.write("=" * 70 + "\n")
...
if __name__ == "__main__":
    ...
    # python3 experiments/cifar100_classify.py